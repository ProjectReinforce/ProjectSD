...
- **N22 다시하기 → 대기실 카운트다운 중 룸 강제 이탈 회귀 fix** (2026-05-13) — known-issues.md N22 의 가설(Voice race)은 빗나갔음. **실제 원인:** `SceneManager.LoadScene("MenuScene")` 직후 모든 패널이 인스펙터 default active 로 잠깐 enable → `TitlePanelController.OnEnable` 이 `NetworkManager.Instance.Connect()` 호출 → InRoom 상태에서 Connect() 가 `IsConnectedAndReady && InLobby` 첫 가드 통과 → `StartConnectAttempt()` → State=Connecting + 워치독 시작 → InRoom 이라 `PhotonNetwork.JoinLobby()` 거부 → 3초 후 워치독 timeout → `HandleAttemptFailed(ClientTimeout)` 의 `PhotonNetwork.Disconnect()` (NetworkManager.Disconnect 우회) → 룸 이탈 + 방 리스트 전환. **픽스:** (1) `NetworkManager.Connect/RetryConnect` 첫 줄 `PhotonNetwork.InRoom` 가드 — 어떤 caller 든 자동 보호. (2) `TitlePanelController.OnEnable` 첫 줄 `InRoom` 가드 — race 자체 차단(이중 안전망). **부산물:** Voice `Operation LeaveRoom (254) not allowed on current server (MasterServer)` LogError 도 `VoiceFollowClient.cs:243` 의 `this.Client.InRoom` 가드 1줄 SDK 패치로 같이 해소 — LeaveRoom/Kick(CloseConnection)/Disconnect 모든 경로 커버. SDK 업그레이드 시 재적용 필요. **UX 보강:** 대기실 뒤로가기 버튼도 카운트다운 활성 시 비활성(캐릭터 변경 버튼 패턴 — Ready 상태에선 허용, 카운트다운만 잠금) — `WaitingRoomPanelController.backButton` SerializeField + `SyncBackButtonLockState` + `OnClickLeaveRoom` 코드 가드. 진단 방법: `PhotonNetwork.NetworkingClient.StateChanged` 구독으로 state 전이 + stack trace 캡처 → Disconnecting 진입 시점의 PUN 내부 trigger 식별. 사용자가 인스펙터에서 WaitingRoomPanel 의 `Back Button` 슬롯에 기존 뒤로가기 Button 할당 완료.
- **Phase 7-5 메타 언락 시스템 + Phase 8-1 Platform 추상화 (Phase A)** (2026-05-12) — [`meta-unlock.md`](../systems/meta-unlock.md) 구현 완료. **3 단위 묶음:** Unit 1 인프라 (Platform Domain 4 + Adapter 2 + Unlock Domain 6 + Adapter 2 + Shared/Domain/Events 2, `RunEventBus` 디커플링) → Unit 2 핵심 기능 (SO 3종 `unlockConditions` 필드 + `SkillDataEditor` 동기화 + `UnlockCatalog` SO + `UnlockTracker` 일괄 평가 + `UnlockSetSync` Photon CustomProperties D5 권위 모델 + 필터 4곳 `SkillManager`/`PlayerWeaponInventory`/`CharacterSelectUI`/`LevelUpManager`) → Unit 3 UI (`ResultPanelUI` 신규 언락 토스트 영역 + `OnNewUnlocks` 구독, 금색 강조). **핵심 기술 결정:** ① 분산형 조건 (D6, 각 SO 옆에 `List<UnlockCondition>`) — Skill/Weapon/Character 분산 + `UnlockCatalog` 는 RefreshCharge 마일스톤 / Cosmetic 슬롯만. ② `[Serializable] struct UnlockCondition` + `UnlockConditionType` enum 5종 — `[SerializeReference]` 미사용 정책 준수. ③ D5 멀티 권위 — `Player.CustomProperties` 의 `mu_skills/mu_weapons/mu_chars/mu_refresh_bonus` int[] 4종으로 자기 셋 공유, 호스트가 `photonView.Owner.ActorNumber` 로 대상 플레이어 셋 조회. ④ D11 런 종료 후 일괄 평가 — `RunEventBus.RunEnded` 발화 시점에 모든 Database 순회 + `OnNewUnlocks(List<UnlockableId>)` 이벤트 발화. ⑤ D13 보스 모든 파티원 카운트 (이미 run-statistics 인프라 공유). **인프라:** `IPlatformService` + `LocalPlatformService` (PlayerPrefs 백엔드) + `PlatformBootstrap` lazy GetOrCreate, Stove/Steam SDK 도입 시 갈아끼움 자동 적용. `MetaProgressStore` (IRunStats 구현, 자기 PC 영구 누적, `LocalStatsRecorder` 의 OnKill/OnBossDefeat/OnDeath hook 에서 `RunEventBus.Raise*` 발화 경유 디커플링). `LevelUpManager.SkillDB` getter 노출 + 초기 충전에 `UnlockSetSync.GetRefreshBonusFor(actor)` 합산. `GameManager` 에 `UnlockCatalog` SerializeField + Awake chain GetOrCreate. **ParrelSync 격리 bridge** (2026-05-12) — `Assets/Scripts/Editor/ParrelSyncBridge.cs` `[InitializeOnLoad]` 가 `LocalPlatformService.CloneSuffixProvider` 셋업 → clone 별 PlayerPrefs key prefix `clone_<arg>.` 적용, ProjectSettings symlink 공유로 양 인스턴스가 같은 PlayerPrefs namespace 쓰던 D5 검증 함정 해소. `MetaUnlockDebugWindow` EditorWindow (Tools > Meta Unlock Debug) — 누적 통계/언락 셋 실시간 + Reset All / Force Unlock / PushSelf / Verbose 로그 토글. **검증 묶음:** Unit 1 (싱글 누적 저장/로드, 재시작 보존), Unit 2 (멀티 D5 — A 언락 스킬이 A 풀에만 등장 / B 미등장, ParrelSync 2 인스턴스 통과), Unit 3 (결과 화면 토스트 표시). **회귀 fix:** `UnlockTracker.LoadUnlockSets` 가 `PlatformBootstrap.GetOrCreate` 보장 — 부트 chain (NetworkManager.Awake → UnlockSetSync → UnlockTracker) 에서 PlatformBootstrap 보다 먼저 호출되는 race 로 Play 재진입 시 영구 데이터 무시되던 버그 해소. **D1 부활 가능 상태** (sourceSkillId 인프라 ✅ + `LocalStatsRecorder.OnKill` 의 sourceSkillId 인자 활용, 향후 SkillData 의 unlockConditions 에 추가 enum 분기로 거의 무비용). audit: architecture-guardian + unity-reviewer + photon-sync-auditor 3종 Critical 0. plan: `~/.claude/plans/synchronous-pondering-taco.md` (작업 후 작성). 별건: 다시하기 카운트 중 Photon Voice race disconnect → known-issues N22 등재 (R14 후행). 상세 [meta-unlock.md](../systems/meta-unlock.md) / [platform-integration.md](../systems/platform-integration.md).
