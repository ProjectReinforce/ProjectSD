...
- 체력 0 → 사망 → **`respawnDelay = 10초`** 자동 부활 타이머 진입 (= **다운 상태**).
- 자동 부활: **체력 `respawnHPRatio = 0.5` (50%) 상태로 안전 지점 스폰**.
- **협력 부활 (R16, 미구현, 2026-05-07 결정):** 다운 상태 동안 아군이 1.5m 이내에서 상호작용 키를 **2초 홀드** 시 즉시 부활. HP 50%, 시신 자리 그대로. 부활자가 이동/공격 시 캔슬, 데미지로는 캔슬되지 않음. 솔로/연결 끊긴 상황에서도 자동 부활은 그대로 작동. 잔여 작업 = [implementation-roadmap.md § R16](../architecture/implementation-roadmap.md).
