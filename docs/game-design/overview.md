...
- **획득:** 엘리트형 드롭 (일정 확률). 모든 플레이어에게 보이며 선착순. 최대 2개.
- **속성:** 얼음(슬로우) / 불(화상 DoT) / 번개(주변 전이 약한 데미지)
- **조합:** 2개 동시 발현 + 특정 조합 히든 효과. 예) 얼음+번개 → 슬로우 적에게 번개 전이는 치명타.
- **런타임 주입:** [systems/trigger-effects.md](../systems/trigger-effects.md) 의 `AddRuntimeEffect(essence_*, …)` 규약 사용.

## 8. 무기 시스템

- **획득:** 모든 적 드롭 (매우 낮은 확률). 선착순.
- **능력치 부여:** 장비마다 스탯 추가 (LoL 아이템 스타일).
- **조합:** 인벤토리 순서대로 체크. 원하는 조합을 전략적으로 우선 선택하도록 유도.
- **TBD:** 슬롯 수, 조합 레시피 상세 (설정 가능).

## 9. 퀘스트 / 능력치 / 기타

- **퀘스트:** 맵 고정 + 랜덤 혼합 위치 생성. 종류: 목표물 처치 / 시간 내 킬 / 낙하 공격 n회 피하기. 보상: 능력치(스탯 부스트) 등급제 선택지.
- **능력치 시스템:** 레벨업 선택지(모든 스킬 슬롯 차고 만렙일 때) / 퀘스트 보상. **등급 체계: 일반 / 희귀 / 영웅 / 전설** 4단계.
- **기타 아이템:** 자석(맵 경험치 전부 흡수), 물약(체력 회복).
